...
const Select: React.FC<ISelect> = ({ name, options, label, value, placeholder, inputClass, labelClass }) => {
    const { handleBlur, handleChange, values } = useFormikContext()

    return (
        <div className="">
            {!!label && <label htmlFor={name} className={classNames("text-[15px] text-[#040404]", labelClass)}>{label}</label>}
            <select
                onChange={handleChange}
                onBlur={handleBlur}
                name={name}
                id={name}
                className={classNames("py-3 px-4 leading-4 text-[#040404] placeholder:text-[#8B8B8B] border-[#E2E2EC] mt-2 w-full rounded bg-[#F7F8FF]", inputClass)}
